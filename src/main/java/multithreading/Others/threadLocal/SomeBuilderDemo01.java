...
public class SomeBuilderDemo01 {
    ...
    public static class SomeBuilder01 {
        ...
        public void build() {
            ...
            if (!counters.containsKey(Thread.currentThread().getName())) counters.put(Thread.currentThread().getName(), 0);
        ...
        public int getCount() { return counters.get(Thread.currentThread().getName()); }
    }

    public static class SomeBuilderThread01 extends Thread {
        private SomeBuilder01 builder;

        public SomeBuilderThread01(SomeBuilder01 builder) { this.builder = builder; }


        @Override
        public void run() {

            for (int i = 0; i < Math.random() * 10; i++) { builder.build(); }
